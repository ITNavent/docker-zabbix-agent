...
def del_maintenance(mid):
    ...
    try:
		...
		data = {"jsonrpc": "2.0", "method": "hostgroup.massremove", "params": {"groups": { "groupid": "198" } , "hosts": { "hostid": [hostid] } }, "auth": token, "id": 1}
		req = urllib2.Request(api)
		data_json = json.dumps(data)
		req.add_header('content-type', 'application/json-rpc')
		...
		try:
			...
			print "Removed Host from Maintenance Group: " + hostname
			sys.exit(0)
...
def start_maintenance():
    ...
    data = {"jsonrpc": "2.0", "method": "maintenance.create", "params":
        {"name": "pause_" + hostname, "active_since": now,"maintenance_type": 1,"active_till": until, "hostids": [hostid], "timeperiods":
    ...
    try:
		...
		data = {"jsonrpc": "2.0", "method": "hostgroup.massadd", "params": {"groups": { "groupid": "198" } , "hosts": { "hostid": [hostid] } }, "auth": token, "id": 1}
		req = urllib2.Request(api)
		data_json = json.dumps(data)
		req.add_header('content-type', 'application/json-rpc')
		...
		try:
			...
			print ("Added Host to Maintenance Group: " + hostname)
			sys.exit(0)
