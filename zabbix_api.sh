...
-s -N "http://zabbix.bumeran.biz/api_jsonrpc.php" | jq -r '.result')
echo TOKEN $TOKEN
...
-s -N "http://zabbix.bumeran.biz/api_jsonrpc.php" | jq -r '.result[0].hostid')
echo HOSTID $HOSTID
...
case $1 in
	...
	enable)
		...
		curl -X POST -H 'Content-type:application/json' -d "$UPDATE_BODY" -s -N "http://zabbix.bumeran.biz/api_jsonrpc.php"
		;;
	disable)
